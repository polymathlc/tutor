// Narrow only the Live bookkeeping namespaces in the CURRENT shared rules.
...
export function addLiveServerRules(source) {
  ...
  const hasRule = source.includes(LIVE_RULE);
  if (!hasRule && /live-server-only-v1|isLiveServerPath/.test(source))
    throw new Error('Existing Live protection differs; review it before changing rules.');
  ...
  const protectedAlready = match[3] === ' && !isLiveServerPath()';
  if (hasRule !== protectedAlready)
    throw new Error('Incomplete Live protection; refusing to guess a repair.');
  if (hasRule) return source;
  const narrowed = match[1] + match[2] + ' && !isLiveServerPath()' + match[4];
  ...
  updated = updated.slice(0, index) + LIVE_RULE + updated.slice(index);
  const restored = updated.replace(LIVE_RULE, '').replace(narrowed, match[0]);
